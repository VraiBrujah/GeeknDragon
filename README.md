...
The contact form uses PHP's built-in `mail()` function to send messages from `no-reply@geekndragon.com`. Ensure your hosting environment permits outbound email.
...
2. Copy `.env.example` to `.env` and fill in `SNIPCART_API_KEY`, `SNIPCART_SECRET_API_KEY`, `SHIPPING_SECRET`, and `ORDER_SECRET`.
   The application automatically loads this file; export the variables manually if you need them in your shell.
