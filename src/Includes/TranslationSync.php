...
class TranslationSync
{
    ...
    private function detectLanguagesFromCsv(array $headers): array
    {
        ...
        foreach ($headers as $header) {
            if (preg_match('/^(\w+)_([a-zA-Z]{2})$/i', $header, $matches)) {
                $languages[] = strtolower($matches[2]);
            }
        }
    ...
    {
        try {
            $jsonPath = dirname(__DIR__, 2) . "/lang/{$lang}.json";
    ...
    {
        try {
            $jsonPath = dirname(__DIR__, 2) . "/lang/{$langCode}.json";
            if (file_exists($jsonPath)) {
                return ['success' => false, 'message' => "Fichier {$langCode}.json existe déjà"];
            }
            $frPath = dirname(__DIR__, 2) . "/lang/fr.json";
    ...
    {
        try {
            $i18nPath = dirname(__DIR__, 2) . '/i18n.php';
