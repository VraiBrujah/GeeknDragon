...
$title = __('meta.contact.title', 'Demande de devis | Geek & Dragon');
$metaDescription = __('meta.contact.desc', 'Demande de devis et informations pour vos projets immersifs.');
$active = 'contact';
$extraHead = '';
...
<main id="main" class="pt-[var(--header-height)] flex items-center justify-center min-h-screen px-4">
  <div class="parchment rounded-3xl p-10 md:p-14 w-full max-w-2xl text-gray-900">
    ...
    <form action="<?= htmlspecialchars(langUrl('/contact'), ENT_QUOTES, 'UTF-8'); ?>" method="POST" class="space-y-6" novalidate>
      ...
      <div>
        ...
        <input id="name" name="name" type="text" required minlength="2" maxlength="120" value="<?= htmlspecialchars($old['name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="Ex. Jean Dupont" class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
      </div>

      <div>
        <label for="email" class="block text-[#4b3e2c] font-semibold mb-2" data-i18n="contact.form.email">Adresse e-mail *</label>
        <input id="email" name="email" type="email" required maxlength="160" value="<?= htmlspecialchars($old['email'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="vous@exemple.com" class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
      </div>

      <div>
        <label for="phone" class="block text-[#4b3e2c] font-semibold mb-2" data-i18n="contact.form.phone">Téléphone</label>
        <input id="phone" name="phone" type="tel" maxlength="40" value="<?= htmlspecialchars($old['phone'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="Optionnel" class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
      </div>
      ...
      <div>
        <label for="message" class="block text-[#4b3e2c] font-semibold mb-2" data-i18n="contact.form.message">Détail de la demande *</label>
        <textarea id="message" name="message" rows="6" required maxlength="<?= $maxChars; ?>" placeholder="Dites-nous tout..." class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600"><?= htmlspecialchars($old['message'] ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
      ...
      <button type="submit" class="btn w-full bg-indigo-700 hover:bg-indigo-600 text-white font-semibold py-3 rounded-full transition" data-i18n="contact.form.submit">Envoyer ma demande</button>
    </form>

    <div class="text-center mt-10 text-sm">
      <p class="txt-court"><strong>Brujah</strong> — <span data-i18n="contact.info.roleCommunity">Responsable produit & communauté</span></p>
