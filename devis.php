...
<body>
  ...
  <main id="main" class="pt-32 flex items-center justify-center min-h-screen px-4">
    <div class="parchment rounded-3xl p-10 md:p-14 w-full max-w-2xl text-gray-900">
      ...
      <form action="<?= htmlspecialchars(langUrl('/devis')) ?>" method="POST" class="space-y-6" novalidate>
        <?php if (!empty($errors)) : ?>
        ...
        <div>
          ...
          <input id="name" name="name" type="text" required minlength="2" maxlength="120" value="<?= htmlspecialchars($old['name'] ?? '') ?>" class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
        </div>
        <div>
          <label for="email" class="block text-[#4b3e2c] font-semibold mb-2" data-i18n="quote.form.email">Adresse e-mail *</label>
          <input id="email" name="email" type="email" required maxlength="160" value="<?= htmlspecialchars($old['email'] ?? '') ?>" class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
        </div>
        <div>
          <label for="phone" class="block text-[#4b3e2c] font-semibold mb-2" data-i18n="quote.form.phone">Téléphone</label>
          <input id="phone" name="phone" type="tel" maxlength="40" value="<?= htmlspecialchars($old['phone'] ?? '') ?>" class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea id="message" name="message" rows="6" required maxlength="<?= $maxChars ?>" class="w-full rounded-md border border-gray-300 p-3 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-600"><?= htmlspecialchars($old['message'] ?? '') ?></textarea>
          <div class="text-sm text-gray-500 mt-1"><span id="char-count">0</span> / <?= $maxChars ?> caractères</div>
        </div>
        <div class="text-sm text-gray-600 bg-gray-50 p-4 rounded" data-i18n="quote.form.consent">En envoyant ce formulaire, vous acceptez que nous utilisions vos informations pour vous répondre par email.</div>
