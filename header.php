...
function renderNav(array $items, string $active, bool $mobile = false): void {
  foreach ($items as $href => $item) {
    $class = 'txt-court transition-colors duration-200 ' . navClass($item['slug'], $active);
    if (isset($item['children']) && !$mobile) {
      echo '<li class="relative group">';
      echo '<a href="' . $href . '" class="' . $class . '" data-i18n="' . $item['i18n'] . '">' . $item['label'] . '</a>';
      echo '<ul class="absolute left-0 top-full hidden group-hover:flex flex-col bg-gray-900/80 p-2 rounded z-10">';
...
<header class="backdrop-blur bg-gradient-to-r from-gray-900/80 to-gray-800/60 shadow-lg fixed top-0 w-full z-50">
  <div class="max-w-7xl mx-auto flex justify-between items-center p-4">
    ...
    <div class="flex items-center space-x-8">
      ...
      <?php if ($snipcartKey): ?>
      <!-- Snipcart -->
      <button class="snipcart-customer-signin txt-court uppercase tracking-wide hover:text-indigo-400 transition-colors duration-200" data-i18n="nav.account">
