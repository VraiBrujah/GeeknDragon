...
/* ========================================================================
   COMPTE — Modale d'accès au portail client
   ===================================================================== */
document.addEventListener('DOMContentLoaded', () => {
  const desktopToggle = document.getElementById('gd-account-toggle');
  const mobileToggle = document.getElementById('gd-account-toggle-mobile');
  const toggles = [desktopToggle, mobileToggle].filter(Boolean);
  if (!toggles.length) return;

  /**
   * Crée la modale de compte si elle n'existe pas déjà.
   * On injecte le HTML une seule fois afin de centraliser la gestion et
   * faciliter les traductions via window.i18n.
   */
  const ensureModal = () => {
    let container = document.getElementById('gd-account-modal');
    if (container) return container;

    container = document.createElement('div');
    container.id = 'gd-account-modal';
    container.className = 'gd-modal-overlay';
    container.setAttribute('aria-hidden', 'true');
    container.setAttribute('hidden', 'hidden');
    container.innerHTML = `
      <div class="gd-modal" role="dialog" aria-modal="true" aria-labelledby="gd-account-title" aria-describedby="gd-account-description">
        <div class="gd-modal-content" tabindex="-1">
          <header class="gd-modal-header">
            <h2 id="gd-account-title" class="gd-modal-title" data-i18n="account.modalTitle">Espace client Geek &amp; Dragon</h2>
            <button type="button" class="gd-modal-close" data-account-close>
              <span class="sr-only" data-i18n="account.close">Fermer</span>
          ...
          <div class="gd-modal-body">
            <div class="gd-account-welcome">
              <p id="gd-account-description" data-i18n="account.modalDescription">Connectez-vous pour suivre vos commandes, consulter vos factures et retrouver vos favoris.</p>
            </div>
            <div class="gd-account-actions">
              <button type="button" class="gd-account-action gd-account-action-primary" data-account-action="dashboard" data-focus-first>
                <span data-i18n="account.ctaDashboard">Ouvrir mon espace client</span>
              </button>
              <a class="gd-account-action gd-account-action-secondary" data-account-link href="/compte.php">
                <span data-i18n="account.ctaPage">Consulter la page Compte</span>
              </a>
            </div>
            <p class="gd-account-hint" data-i18n="account.modalHint">Astuce : si vous n'avez pas encore de compte, ouvrez la page dédiée pour en créer un ou demander de l'aide.</p>
          </div>
        </div>
      </div>
    `;
  ...
  const modalContent = overlay.querySelector('.gd-modal-content');
  const closeButton = overlay.querySelector('[data-account-close]');
  const dashboardButton = overlay.querySelector('[data-account-action="dashboard"]');
  const fallbackLink = overlay.querySelector('[data-account-link]');
  const focusFirst = overlay.querySelector('[data-focus-first]');
  const trap = window.GD.focusTrap(modalContent);

  let lastFocused = null;
  let isOpen = false;
  let hideTimer = null;
  const accountFallbackUrl = (fallbackLink && fallbackLink.getAttribute('href')) || '/compte.php';

  /**
   * Met à jour l'état aria des déclencheurs pour refléter la visibilité
   * de la modale et renforcer l'accessibilité clavier/lecteur d'écran.
   */
  ...
  const openModal = () => {
    ...
    setToggleState(true);
    trap.mount();
    const target = focusFirst || closeButton || modalContent;
    ...
    dashboardButton.addEventListener('click', (event) => {
      ...
      try {
        if (window.Snipcart && window.Snipcart.api) {
          if (window.Snipcart.api.theme && typeof window.Snipcart.api.theme.openAccount === 'function') {
            window.Snipcart.api.theme.openAccount();
            handled = true;
          } else if (window.Snipcart.api.customer && typeof window.Snipcart.api.customer.openAccount === 'function') {
            window.Snipcart.api.customer.openAccount();
            handled = true;
          }
        }
      } catch (error) {
        if (window.GD && typeof window.GD.log === 'function') {
          window.GD.log('Erreur ouverture portail client', error);
        }
      }

      if (!handled) {
        window.location.assign(accountFallbackUrl);
        return;
      }
