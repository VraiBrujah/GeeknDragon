// ========================================================================
// GEEK & DRAGON - E-COMMERCE NATIF HYBRIDE
// Interface DnD native + Synchronisation Snipcart transparente
// Architecture en îlots avec hydratation partielle + API Snipcart
// ======================================================================== 
...
(function() {
  ...
  function addToCart(productData) {
    ...
    // SYNCHRONISATION SNIPCART TRANSPARENTE
    syncWithSnipcart(productData, 'add');
  ...
  function updateItemQuantity(itemId, variants, newQuantity) {
    ...
    if (newQuantity <= 0) {
      ...
      syncWithSnipcart({id: itemId, variants: variants, quantity: newQuantity}, 'update');
    }
  ...
  function removeFromCart(itemId, variants = {}) {
    ...
    if (itemIndex !== -1) {
      ...
      // SYNCHRONISATION SNIPCART
      syncWithSnipcart({id: removedItem.id, variants: removedItem.variants}, 'remove');
  ...
  /**
   * Procède au checkout avec Snipcart
   */
  function proceedToCheckout() {
    ...
    console.log('Redirection vers checkout Snipcart avec:', state.cart);
    
    // Synchroniser tout le panier avec Snipcart si nécessaire
    ensureSnipcartSync();
    
    // Ouvrir le checkout Snipcart après un délai pour s'assurer de la sync
    setTimeout(() => {
      if (window.Snipcart && window.Snipcart.api) {
        window.Snipcart.api.modal.show();
      } else {
        console.warn('Snipcart non disponible, fallback vers page checkout');
  ...
  /**
   * Synchronise une action avec Snipcart
   */
  function syncWithSnipcart(productData, action) {
    if (!window.Snipcart || !window.Snipcart.api) {
      console.warn('Snipcart non disponible pour synchronisation');
      return;
    }
    ...
    try {
      switch (action) {
        case 'add':
          // Créer un bouton Snipcart virtuel et le déclencher
          addToSnipcart(productData);
          break;
        case 'update':
          updateInSnipcart(productData);
          break;
        case 'remove':
          removeFromSnipcart(productData);
          break;
      }
    } catch (error) {
      console.warn('Erreur synchronisation Snipcart:', error);
    }
  }

  /**
   * Ajoute un article à Snipcart via API
   */
  function addToSnipcart(productData) {
    const snipcartData = {
      id: productData.id,
      ...
      image: productData.image || '/images/optimized-modern/webp/brand-geekndragon-main.webp'
    };

    // Ajouter les variantes comme custom fields
    if (productData.variants && Object.keys(productData.variants).length > 0) {
      ...
      Object.entries(productData.variants).forEach(([key, value]) => {
        if (customFieldIndex <= 3) { // Snipcart supporte jusqu'à 3 custom fields
          snipcartData[`customField${customFieldIndex}Name`] = key;
          snipcartData[`customField${customFieldIndex}Value`] = value;
          customFieldIndex++;
        }
      });
    }

    console.log('🔄 Sync Snipcart ADD:', snipcartData);
    window.Snipcart.api.items.add(snipcartData);
  }

  /**
   * Met à jour un article dans Snipcart
   */
  function updateInSnipcart(productData) {
    const items = window.Snipcart.api.items.all();
    const itemToUpdate = items.find(item => 
      item.id === productData.id && 
      JSON.stringify(getItemVariants(item)) === JSON.stringify(productData.variants)
    );

    if (itemToUpdate) {
      console.log('🔄 Sync Snipcart UPDATE:', productData);
      window.Snipcart.api.items.update(itemToUpdate.uniqueId, {
  ...
  /**
   * Supprime un article de Snipcart
   */
  function removeFromSnipcart(productData) {
    const items = window.Snipcart.api.items.all();
    ...
    if (itemToRemove) {
      console.log('🔄 Sync Snipcart REMOVE:', productData);
      window.Snipcart.api.items.remove(itemToRemove.uniqueId);
    }
  }
   ...
   */
  function getItemVariants(snipcartItem) {
    const variants = {};
    if (snipcartItem.customFields) {
      snipcartItem.customFields.forEach(field => {
        if (field.name && field.value) {
          variants[field.name] = field.value;
        }
      });
    }
    return variants;
  ...
  /**
   * S'assure que tout le panier est synchronisé avec Snipcart
   */
  function ensureSnipcartSync() {
    if (!window.Snipcart || !window.Snipcart.api) {
      console.warn('Snipcart non disponible pour synchronisation complète');
      return;
    }

    console.log('🔄 Synchronisation complète avec Snipcart...');
    
    // Vider Snipcart d'abord
    window.Snipcart.api.items.clear();
    
    // Re-ajouter tous nos articles
  ...
  /**
   * Initialise l'intégration Snipcart
   */
  function initSnipcartIntegration() {
    if (window.Snipcart && window.Snipcart.api) {
      ...
      document.addEventListener('snipcart.ready', setupSnipcartListeners);
  ...
  /**
   * Configure les listeners Snipcart pour maintenir la synchronisation
   */
  function setupSnipcartListeners() {
    if (!window.Snipcart?.api) return;

    // Écouter les changements Snipcart pour maintenir la sync (optionnel)
    window.Snipcart.api.on('cart.confirmed', () => {
      console.log('✅ Commande confirmée - vider notre panier local');
      clearCart();
    });
  ...
  function init() {
    ...
    // INITIALISATION SNIPCART HYBRIDE
