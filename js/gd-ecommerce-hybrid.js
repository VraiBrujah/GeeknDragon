...
// Interface DnD Role-Play + Snipcart Fonctionnel
// ========================================================================
...
(function() {
  ...
  const DND_CLASSES = [
    { value: 'artificer', label: '🔧 Artificier', description: 'Maître des inventions magiques' },
    { value: 'barbarian', label: '⚔️ Barbare', description: 'Guerrier sauvage et puissant' },
    { value: 'bard', label: '🎵 Barde', description: 'Artiste magique et polyvalent' },
    { value: 'cleric', label: '⚕️ Clerc', description: 'Prêtre aux pouvoirs divins' },
    { value: 'druid', label: '🌿 Druide', description: 'Gardien de la nature' },
    { value: 'fighter', label: '🛡️ Guerrier', description: 'Combattant expert et discipliné' },
    { value: 'monk', label: '👊 Moine', description: 'Ascète aux arts martiaux mystiques' },
    { value: 'paladin', label: '⚡ Paladin', description: 'Champion divin et protecteur' },
    { value: 'ranger', label: '🏹 Rôdeur', description: 'Pisteur et chasseur des terres sauvages' },
    { value: 'rogue', label: '🗡️ Voleur', description: 'Expert en discrétion et agilité' },
    { value: 'sorcerer', label: '✨ Sorcier', description: 'Magie innée et instinctive' },
    { value: 'warlock', label: '👹 Sorcier des pactes', description: 'Pouvoir octroyé par un patron' },
    { value: 'wizard', label: '🔮 Magicien', description: 'Érudit de la magie arcanique' }
  ];
  ...
  function extractProductVariants(button) {
    ...
    if (multiplierSelect && multiplierSelect.value && multiplierSelect.value !== '1') {
      variants.Multiplicateur = multiplierSelect.options[multiplierSelect.selectedIndex].text.trim();
    }

    // Langues
    const languageSelect = container.querySelector('select[data-type="language"]');
    if (languageSelect && languageSelect.value) {
      variants.Langue = languageSelect.options[languageSelect.selectedIndex].text.trim();
    }
            ...
            <div class="gd-modal-body">
              <div class="gd-cart-hybrid">
                ...
                <div class="gd-cart-actions" style="display: flex; gap: 1rem; margin-top: 1.5rem;">
                  ...
                  <button class="gd-btn gd-btn-primary gd-btn-full snipcart-checkout">
              ...
              <div style="background: var(--gd-bg-primary); border: 1px solid var(--gd-border); border-radius: var(--gd-radius-lg); padding: var(--gd-space-lg); margin-bottom: var(--gd-space-md);">
                <div style="display: flex; gap: var(--gd-space-md); align-items: flex-start;">
                  <div style="flex: 1;">
                    ...
                    ${variantsHtml ? `<div class="gd-cart-item-variants"><div class="gd-variant-label">Caractéristiques :</div>${variantsHtml}</div>` : ''}
          ...
          contentDiv.innerHTML = `
            ...
            <div class="gd-cart-summary-enhanced">
              <div class="gd-cart-summary-row">
                <span class="gd-cart-summary-label">Articles (${itemCount})</span>
  ...
  function attachEventListeners() {
    ...
    // Fermeture des modales - Délégation d'événements
    document.addEventListener('click', (e) => {
      ...
      if (e.target.classList.contains('gd-modal-close') || e.target.closest('.gd-modal-close')) {
        const button = e.target.closest('.gd-modal-close');
  ...
  function initSnipcartIntegration() {
    ...
    if (window.Snipcart) {
      // Snipcart déjà chargé
      setupSnipcartListeners();
    } else {
      // Attendre le chargement de Snipcart
