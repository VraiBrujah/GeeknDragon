// ========================================================================
// GEEK & DRAGON - SYSTÈME HYBRIDE TRANSPARENT
// Interface DnD native + Snipcart transparent en arrière-plan
// L'utilisateur voit seulement la belle interface DnD, Snipcart est caché
// ========================================================================
...
(function() {
  ...
  // Classes DnD officielles complètes (toutes les 13 classes)
  const DND_CLASSES = [
    ...
    { value: 'rogue', label: '🗡️ Voleur', description: 'Expert en discrétion, agilité et coups sournois' },
    { value: 'sorcerer', label: '✨ Sorcier', description: 'Magie innée coulant dans ses veines' },
    { value: 'warlock', label: '👹 Occultiste', description: 'Pouvoir magique octroyé par un patron surnaturel' },
  ...
  function extractProductVariants(button) {
    ...
    if (multiplierSelect && multiplierSelect.value && multiplierSelect.value !== '1') {
      ...
      if (selectedOption) {
        // Nettoyer le texte : enlever les caractères invisibles et tirets
        const cleanText = selectedOption.text.replace(/[‎\u200B-\u200F\u2028-\u202F—–\s]*—\s*/, '').trim();
        variants.Multiplicateur = cleanText;
      }
    }
    ...
    if (button.dataset.itemCustom1Name && button.dataset.itemCustom1Value) {
      ...
      const cleanValue = customValue.replace(/[‎\u200B-\u200F\u2028-\u202F—–\s]*—\s*/, '').trim();
  ...
  // ========================================================================
  // INTÉGRATION SNIPCART TRANSPARENTE
  // ========================================================================
    ...
    if (snipcartItem.customFields && Array.isArray(snipcartItem.customFields)) {
      snipcartItem.customFields.forEach(field => {
        if (field.name && field.value) {
          // Nettoyer les valeurs
          const cleanValue = field.value.replace(/[‎\u200B-\u200F\u2028-\u202F—–\s]*—\s*/, '').trim();
  ...
  function attachEventListeners() {
    ...
    // Fermeture des modales - Gestion améliorée des boutons X
    document.addEventListener('click', (e) => {
      // Clic sur l'overlay
      ...
      // Clic sur le bouton fermer ou son contenu (SVG)
  ...
  function init() {
    ...
    console.log('🏰 Initialisation du système hybride transparent Geek & Dragon');
    ...
    console.log('✅ Système hybride transparent initialisé avec succès');
  }
