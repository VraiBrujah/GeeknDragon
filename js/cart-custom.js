...
class GeeknDragonCart {
  ...
  init() {
    ...
    if (!this.cartToggle || !this.cartPanel) {
      this.showError('GeeknDragon Cart: Elements required not found');
      return;
    }
  ...
  async addFromButton(button) {
    ...
    try {
      ...
      // Feedback visuel
      this.constructor.showAddedFeedback(button);
    } catch (error) {
  ...
  async addItem(item) {
    try {
      ...
      this.showError('Mode hors ligne, gestion locale du panier');
  ...
  /**
     ...
     */
  addItemLocally(item) {
    const itemKey = this.constructor.generateItemKey(item.id, item.options || {});
  ...
  updateCartTotal() {
    ...
    if (this.cartTotal) {
      this.cartTotal.textContent = this.constructor.formatPrice(total);
    }
  }
  ...
  async reloadCartContent() {
    try {
      ...
      this.showError('Impossible de recharger le contenu du panier');
    }
  }
  ...
  loadFromSession() {
    try {
      ...
      this.showError('Impossible de charger le panier depuis la session');
    }
  }
  ...
  /**
     ...
     */
  static generateItemKey(productId, options) {
  ...
  /**
     ...
     */
  static showAddedFeedback(button) {
    const btn = button;
  ...
  showError(message) {
    ...
    const container = document.getElementById('gd-cart-error');
    if (container) {
