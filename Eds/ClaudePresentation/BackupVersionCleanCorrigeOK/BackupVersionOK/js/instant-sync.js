...
class InstantSync {
    ...
    setupInstantListeners() {
        ...
        editableFields.forEach(field => {
            this.registerFieldListeners(field);
        });
    }

    /**
     * Enregistrement : écouteurs sur un champ spécifique (utilisé pour les éléments dynamiques)
     * @param {HTMLElement} field - Élément à enregistrer pour la synchronisation
     */
    registerFieldListeners(field) {
        // Vérification : éviter l'enregistrement multiple
        if (field.hasAttribute('data-sync-registered')) {
            return;
        }
        ...
        // Marquage : champ déjà enregistré
        field.setAttribute('data-sync-registered', 'true');
        ...
        // Initialisation : valeur dans le cache
        const value = this.extractFieldValue(field);
        this.fieldValues.set(fieldName, value);
        
        console.log(`📡 Champ dynamique enregistré: ${fieldName}`);
    }

    /**
     * Enregistrement : nouveau champ créé dynamiquement
     * @param {HTMLElement} field - Nouvel élément avec data-field
     */
